...
/* Un ÚNICO broker de señalización para los dos lados: el broker público
   por defecto de PeerJS (0.peerjs.com). Probar varios brokers distintos
   sin un canal previo para ponerse de acuerdo hace que host e invitado
   acaben en servidores distintos y no se encuentren nunca — peor que un
   solo broker. Ante un fallo transitorio se REINTENTA el mismo. */
/* ICE: STUN (Google/Cloudflare) + TURN gratuito (OpenRelay). Sin un TURN
   que releve el tráfico, dos dispositivos en redes móviles / detrás de
   CGNAT abren la sala en el broker pero el canal de datos WebRTC nunca
   llega a establecerse: se queda en "conectando" y acaba fallando. Se
   incluye :443 y ?transport=tcp para redes que solo dejan salir el 443. */
const ICE_SERVERS = [
...
function createRealTransport(){
  ...
  function hostCreateRoom(){
    ...
    function attempt(retry){
      ...
      let settled = false;
      const p = new window.Peer(PEER_PREFIX+code, peerOpts());
      const to = setTimeout(()=>{
