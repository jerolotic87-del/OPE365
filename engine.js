...
/* Nº de ítems para un presupuesto de minutos. Objetivo aproximado, NO un tope
   duro: se usa el ritmo real observado (EMA de los ms medidos). El único
   recorte es un suelo absoluto de P.minSecPerItemHard segundos por ítem, que
   sólo existe para evitar cifras irreales si el ritmo medido fuese absurdo. */
function itemsForMinutes(minutes){
  const sec = observedSecPerItem("q");
  const raw = Math.round(minutes * 60 / sec);
  const ceil = Math.round(minutes * 60 / P.minSecPerItemHard);
