...
/* ---------------------------------------------------------------
   TEMARIO — las pestañas de Word 365 como una lista con el estado
  ...
  <div class="view">
    ...
    <div class="tm-acc-list">
      ${O.TAXONOMY_SECTIONS.map(sec=>{
        ...
        const barB = scm ? { total:scm.total,
          asentado:scm.topics.filter(t=>t.mastery==="asentado").length,
          consolidando:scm.topics.filter(t=>t.mastery==="consolidando").length,
...
/* abreviaturas tras las que NO se debe partir en frase nueva */
const EXPL_ABBR = /(?:\bp|\bej|\bp[aá]g|\bn[uú]m|\bart|\bcap|\bfig|\bvol|\bpto|\bSr|\bSra|\bDr|\baprox|\bm[aá]x|\bm[ií]n)\.$/i;
/* parte una explicación en segmentos (una idea por línea). Divide en
...
function splitExpl(raw){
  const parts = raw.split(/\s+·\s+/).flatMap(chunk=>{
    ...
    chunk.split(/(?<=[.;])\s+(?=[«"“¿(A-ZÁÉÍÓÚÑ]|Ctrl\b|Alt\b|May[uú]s\b|Mays\b|Win|F\d)/)
      .forEach(seg=>{
        if(buf && (EXPL_ABBR.test(buf.trim()) || seg.trim().length < 12)){ buf += " " + seg; }
        else { if(buf) out.push(buf.trim()); buf = seg; }
      });
...
function explInline(seg){
  return kbdify(O.escapeHtml(seg).replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>"));
}
/* explicación -> HTML. Con varios elementos se pinta como lista con una
   ligera separación (menos fatiga visual); con uno solo, texto corrido. */
function explHtml(s){
  const segs = splitExpl(cleanExplic(s));
  if(segs.length >= 2)
    return `<ul class="expl-list">${segs.map(x=>`<li>${explInline(x)}</li>`).join("")}</ul>`;
  return explInline(segs[0] || "");
}
...
function showFeedback(el, q, resp){
  ...
  const segs = splitExpl(full);
  // recorte: lista larga -> se ocultan los puntos a partir del 4.º; párrafo
  // largo -> line-clamp. Un botón debajo lo despliega en el sitio.
  const isList = segs.length >= 2;
  const longish = isList ? segs.length >= 4 : full.length > 240;
  ...
  el.innerHTML = `
    <div class="fb ${resp.correct?'fb-ok':'fb-bad'}">
      ...
      ${full ? `<div class="fb-gist${isList?' is-list':''}${longish?' is-clamped':''}">${explHtml(q.explicacion)}</div>` : ''}
...
function renderTemarioDetalle(params){
  ...
  const scm = O.LEB ? O.LEB.sectionConceptsModel(sectionId) : null;
  const topics = (sec.topics || []).map(t=>{
    const cv = scm ? scm.topics.find(x=>x.topicId===t.id) : null;
    return { ...t, n: O.filterQuestions({section:sectionId, topic:t.id}).length, cv };
  });
  ...
  const asentadoN = scm ? scm.asentado : 0;
  const repasoN = scm ? scm.repaso : 0;
  const barB = scm ? { total:scm.total,
    asentado:scm.topics.filter(t=>t.mastery==="asentado").length,
    consolidando:scm.topics.filter(t=>t.mastery==="consolidando").length,
    aprendiendo:scm.topics.filter(t=>t.mastery==="aprendiendo").length,
  ...
  <div class="view view-narrow">
    ...
    <button class="btn btn-solid btn-lg btn-block" id="td-preguntas" style="margin-bottom:var(--sp-6);">
      ${icon('play')} Practicar toda la pestaña · ${qCount}
    </button>

    ${topics.length ? `
    <div class="section-block">
      <div class="section-title"><h3>Bloques</h3><span class="section-hint">tu estado en cada uno</span></div>
      <div class="nav-list">
        ${topics.map(t=>{
          const cv = t.cv;
          const tone = cv && cv.seen ? cv.masteryTone : "neutral";
          const stlbl = cv ? (cv.seen ? cv.masteryLabel : "Sin empezar") : "";
          return `<button class="nav-row concept-row" data-topic="${t.id}" ${t.n?'':'disabled'}>
          <span class="state-pip tone-${tone}"></span>
          <span class="nr-body">
            <span class="nr-title">${O.escapeHtml(t.name)}</span>
            <span class="nr-reason">${stlbl}${cv && cv.reviewLabel?` · <span class="mini-warn">${cv.reviewLabel}</span>`:''} · ${t.n} pregunta${t.n===1?'':'s'}</span>
          </span>
...
function renderPokerReveal(r){
  ...
  return `
    ...
    <div class="surface pad-5" style="margin-top:var(--sp-4);">
      ...
      ${q.explicacion ? `<div style="font-size:12.5px; color:var(--text-2); margin-top:10px; border-top:1px solid var(--border); padding-top:10px;">${explHtml(q.explicacion)}</div>` : ''}
    </div>
